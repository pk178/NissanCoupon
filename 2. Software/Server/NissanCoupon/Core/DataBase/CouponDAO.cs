    ...
    public class CouponDAO
    {
        ...
        public static List<DealerRedeemed> GetCouponByDealer(DealerInfo Dealer, DateTime FromTime, DateTime ToTime)
        {
            try
            {
                ...
                var Filter = Builders<CouponInfo>.Filter.And(
                        Builders<CouponInfo>.Filter.Gte(x => x.RedeemedDate, FromTime),
                        Builders<CouponInfo>.Filter.Lte(x => x.RedeemedDate, ToTime),
                        Builders<CouponInfo>.Filter.Ne(x => x.RedeemedByDealer, string.Empty),
                        Builders<CouponInfo>.Filter.Ne(x => x.RedeemedByDealer, "Canceled"),
                        Builders< CouponInfo>.Filter.Or(
                            Builders<CouponInfo>.Filter.Eq(x => x.RedeemedByDealer, Dealer.Name),
                            Builders<CouponInfo>.Filter.Where(x => Dealer.Name == "Nissan Viet Nam"),
                            Builders<CouponInfo>.Filter.Where(x => Dealer.Name == "Nationwide")));
            ...
            {
                NissanCouponLibrary.Utils.Log.LogError("GetCouponByDealer", "", ex.Message);
            }
        ...
        public static List<CouponInfo> GetAllCoupon(DateTime FromTime, DateTime ToTime)
        {
            try
            {
                var collection = DAOManager._database.GetCollection<CouponInfo>("CouponInfo");
                var Filter = Builders<CouponInfo>.Filter.And(
                        Builders<CouponInfo>.Filter.Gte(x => x.PromotionDate, FromTime),
                        Builders<CouponInfo>.Filter.Lte(x => x.PromotionDate, ToTime));
            ...
            {
                NissanCouponLibrary.Utils.Log.LogError("GetAllCoupon", string.Format("Time : {0} -> {1}", FromTime, ToTime), ex.Message);
            }
